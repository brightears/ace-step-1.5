...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    def get_x0_from_noise(self, zt, vt, t):
        # Defensive: ensure timestep batch matches latent batch (SDE + CFG edge case)
    ...
    def generate_audio(
        ...
        **kwargs,
        ...
        xt = noise
        non_cover_initialized = False
        with torch.no_grad():
            for step_idx, (t_curr, t_prev) in enumerate(iterator):
                if step_idx >= cover_steps and not non_cover_initialized:
                    non_cover_initialized = True
                    if do_cfg_guidance:
