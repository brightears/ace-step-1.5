...
def adg_forward(
    ...
    # Defensive: expand latents to match noise prediction sequence length (CFG edge case)
    if latents.shape[1] != t:
        repeats = t // latents.shape[1]
        if repeats > 1 and t == latents.shape[1] * repeats:
            latents = latents.repeat(1, repeats, 1)
        else:
            latents = latents[:, :t, :]  # truncate if mismatched
